...
class TestBurger:
    ...
    def test_set_buns_sets_correct_bun_in_burger(self, burger, mock_bun):
        # Проверяет, что булочка устанавливается правильно в бургер.
        burger.set_buns(mock_bun)
        assert burger.bun == mock_bun

    def test_add_ingredient_adds_correct_ingredient_to_burger(self, burger, mock_ingredient):
        # Проверяет, что ингредиент добавляется в бургер.
        burger.add_ingredient(mock_ingredient)
        assert mock_ingredient in burger.ingredients

    def test_remove_ingredient_removes_correct_ingredient_from_burger(self, burger, mock_ingredient):
        # Проверяет, что ингредиент удаляется из бургера по индексу.
        burger.add_ingredient(mock_ingredient)
        burger.remove_ingredient(0)
        assert mock_ingredient not in burger.ingredients

    def test_move_ingredient_moves_ingredient_within_burger(self, burger):
    ...
    def test_get_price_calculates_correct_price_of_burger(self, burger, mock_bun, mock_ingredient):
    ...
    def test_get_receipt_returns_correct_receipt_format(self, burger, mock_bun, mock_ingredient):
